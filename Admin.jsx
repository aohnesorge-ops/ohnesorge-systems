  ...
  return(
    <nav style={{position:"fixed",top:BANNER_H,left:0,right:0,zIndex:150,height:64,display:"flex",alignItems:"center",background:sc?"rgba(7,7,7,.95)":"transparent",backdropFilter:sc?"blur(16px)":"none",borderBottom:sc?"1px solid rgba(255,255,255,.06)":"none",transition:"all .3s"}}>
      <div className="W" style={{display:"flex",alignItems:"center",gap:24,width:"100%"}}>
        ...
        <button onClick={()=>setOpen(o=>!o)} className="HAM" style={{display:"none",background:"none",border:"none",cursor:"pointer",color:T,padding:6,marginLeft:"auto"}}>
  ...
  return(
    <section className="S"><div className="W" ref={ref}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:16,marginBottom:4}}>
        {["🔨 Handwerk","🍕 Gastronomie","⚖️ Recht & Beratung","💅 Beauty & Wellness","🏠 Immobilien","🚗 KFZ","🏋️ Fitness","🌿 Gesundheit"].map(b=>(
          <span key={b} style={{fontSize:12,background:B3,border:`1px solid ${BR}`,borderRadius:20,padding:"5px 12px",color:TD}}>{b}</span>
        ))}
      </div>
      <div className="G3 MT">
...
function SchnellcheckTool(){
  ...
  if(result)return(
    <div style={{animation:"fadeUp .4s ease both"}}>
      ...
      {!sent?(<div><div style={{fontSize:13,fontWeight:600,color:T,marginBottom:8}}>Ergebnis + kostenloses Gespräch sichern:</div><div style={{display:"flex",gap:8,marginBottom:10}}><input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="ihre@email.de" className="INP" style={{flex:1}}/><button onClick={()=>{if(email.includes("@"))setSent(true);}} className="btn" style={{padding:"10px 16px",fontSize:13,whiteSpace:"nowrap"}}>Senden →</button></div>
        <div style={{textAlign:"center",margin:"8px 0",fontSize:12,color:TF}}>— oder direkt —</div>
        <a href="https://wa.me/49IHRE_NUMMER?text=Hallo%2C%20ich%20habe%20gerade%20den%20Website-Check%20gemacht%20und%20möchte%20mehr%20erfahren." target="_blank" rel="noopener noreferrer" style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,background:"#25d366",color:"#fff",borderRadius:8,padding:"10px 16px",fontSize:13,fontWeight:700,textDecoration:"none",width:"100%"}}>
          ...
          Direkt via WhatsApp — jetzt antworten
        </a>
        <button onClick={()=>{setStep(0);setAns({});setResult(null);setSent(false);}} style={{background:"none",border:"none",color:TF,fontSize:12,cursor:"pointer",marginTop:10,fontFamily:"inherit"}}>↩ Neue Analyse</button></div>)
...
function AutomationVisualizer(){
  const[ref,vis]=useInView(0.05);const[active,setActive]=useState(0);
  useEffect(()=>{
    if(!vis)return;
    // Start immediately, cycle every 900ms
    setActive(0);
    const t=setInterval(()=>setActive(a=>(a+1)%ASTEPS.length),900);
  ...
  return(
    <section className="S"><div className="W" ref={ref}>
      ...
      <div className="G4P MT">
        ...
        {[
          ["01","Tag 1 — Erstgespräch","30 Min. kostenlos. Wir klären was Sie brauchen, ich gebe erste Einschätzung. Kein Kaufdruck."],
          ["02","Tag 2 — Angebot","Am nächsten Werktag: konkretes Festpreis-Angebot per E-Mail. Transparent, keine Überraschungen."],
          ["03","Tag 3–6 — Umsetzung","Sie sehen täglich den Fortschritt. Feedback-Runden inklusive. Sie müssen nichts tun."],
          ["04","Tag 7 — Launch","Live. Online. Kunden kommen. 3 Monate Support inklusive — danach kein Abo, kein Vertrag."],
        ].map(([n,t,d],i)=>(
          <div key={i} className={`PSTEP fade ${vis?"vis":""} d${i+1}`} style={{padding:"0 14px",textAlign:"center"}}>
            ...
            <h3 style={{fontSize:15,fontWeight:700,color:T,marginBottom:8}}>{t}</h3>
...
function Preise({to}){
  ...
  return(
    <section id="preise" className="S Sd"><div className="W" ref={ref}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:0,margin:"24px auto 0",maxWidth:600,background:B3,border:`1px solid ${BR}`,borderRadius:12,overflow:"hidden"}}>
        <div style={{flex:1,padding:"14px 20px",textAlign:"center",borderRight:`1px solid ${BR}`}}>
          <div style={{fontSize:11,fontWeight:600,color:TF,textTransform:"uppercase",letterSpacing:".08em",marginBottom:4}}>Typische Agentur</div>
          <div style={{fontSize:18,fontWeight:800,color:RE,letterSpacing:"-.02em"}}>6.000 – 20.000 €</div>
          <div style={{fontSize:12,color:TF,marginTop:2}}>3–6 Monate Wartezeit</div>
        </div>
        <div style={{flex:1,padding:"14px 20px",textAlign:"center",background:PD,border:`1px solid ${PB}`,margin:-1}}>
          <div style={{fontSize:11,fontWeight:600,color:P,textTransform:"uppercase",letterSpacing:".08em",marginBottom:4}}>Ohnesorge</div>
          <div style={{fontSize:18,fontWeight:800,color:GR,letterSpacing:"-.02em"}}>Ab 890 €</div>
          <div style={{fontSize:12,color:TF,marginTop:2}}>Fertig in 7 Tagen</div>
        </div>
      </div>
...
function ChatWidget(){
  ...
  return(
    <>
      {open&&(
        <div style={{position:"fixed",bottom:76,right:20,width:320,maxHeight:460,background:B2,border:`1px solid rgba(255,255,255,.1)`,borderRadius:16,display:"flex",flexDirection:"column",zIndex:300,boxShadow:"0 24px 64px rgba(0,0,0,.5)",animation:"fadeUp .25s ease"}}>
...
export default function App(){
  ...
  return(
    <>
      <style>{CSS}</style>
      <Nav scrollY={scrollY} to={to}/><Hero to={to}/><StatsBar/><ProblemSection/>
      ...
      <Leistungen to={to}/><Prozess/><Preise to={to}/><FAQ/><ProjektGalerie/><Testimonials/><UeberMich to={to}/><Kontakt/>
